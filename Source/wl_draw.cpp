...
#define NUMPATCHOBJS        17
#define LASTPATCHOBJ        (FIRSTPATCHOBJ+NUMPATCHOBJS)
#define FIRSTPATCHOFFOBJ    700
#define LASTPATCHOFFOBJ     (FIRSTPATCHOFFOBJ+NUMPATCHOBJS)
#define NUMPATCHES          NUMPATCHOBJS
...
unsigned GetPatchNum(boolean useXspot)
{
    ...
    if (spot < FIRSTPATCHOBJ || spot >= LASTPATCHOBJ)
    {
        if (spot < FIRSTPATCHOFFOBJ || spot >= LASTPATCHOFFOBJ)
            return 0;
    }
...
boolean ApplyPatch(int wallpic, boolean useXspot)
{
    ...
    if (spot > 0 && wallpic < DOORWALL) // Don't do door-related textures.
    {
        ...
        /* switch (spot)
        {
            case FIRSTPATCHOBJ+2:
                spot = FIRSTPATCHOBJ+2;
                spot -= FIRSTPATCHOBJ;
                break;
            case FIRSTPATCHOBJ:
            case FIRSTPATCHOBJ+1:
                spot = FIRSTPATCHOBJ;
                spot -= FIRSTPATCHOBJ;
                break;
            case FIRSTPATCHOFFOBJ:
            case FIRSTPATCHOFFOBJ+1:
                spot = FIRSTPATCHOBJ+1;
                spot -= FIRSTPATCHOBJ;
                break;
            default:
                break;
        ...
        else if (spot >= FIRSTPATCHOFFOBJ && spot <= LASTPATCHOFFOBJ)
        {
            spot -= FIRSTPATCHOFFOBJ;
        }
        ...
        scan = PM_GetTexture(PATCHSTART+spot);
        for (pixel=0; pixel<PMPageSize; pixel++,scan++) {
            if (*scan < 255) {
                patchedwall[pixel] = *scan;
            }
...
#ifdef WALLPATCH
    if(lastside==1 && lastintercept==xtile && lasttilehit==tilehit && !(lasttilehit & 0x40) && GetPatchNum(true)==lastpatchnum)
#else
...
#ifdef WALLPATCH
    if(lastside==0 && lastintercept==ytile && lasttilehit==tilehit && !(lasttilehit & 0x40) && lastpatchnum==GetPatchNum(false))
#else
...
{
    byte ceiling=vgaCeiling[gamestate.episode*6+mapon];
...
//
    ...
    for (statptr = &statobjlist[0] ; statptr !=laststatobj ; statptr++)
    {
        ...
        for (int animstat = 0; animsprinfo[animstat].picnum != -1; animstat++)
        {
            ...
            {
                visptr->shapenum += int((frameon/(animsprinfo[animstat].animtics))%animsprinfo[animstat].animframes);
            }
        }
...
//
    ...
    for (i = 0; i<numvisable; i++)
    {
        ...
        {
            if(farthest->transactor != NULL) Scale3DShapeActor(vbuf, vbufPitch,
                                                               farthest->transactor,
                                                               farthest->flags);
...
int weaponscale[NUMWEAPONS] = {SPR_KNIFEREADY, SPR_PISTOLREADY,
    SPR_MACHINEGUNREADY, SPR_CHAINREADY, SPR_RTHROWREADY};
...
void DrawPlayerWeapon (void)
{
    ...
    if (gamestate.weapon != -1)
    {
        ...
        SimpleScaleShape(viewwidth/2+48+gamestate.bobber2,shapenum,viewheight+1+gamestate.bobber);
    }
...
//
    ...
    if (hinttime > 0)
        ...
        strcpy(gamestate.comphint, "");
    
    if (ach_time > 0 && latest_ach != -1)
        DrawAchievement(latest_ach);
    else if (ach_time <= 0)
    {
        latest_ach = -1;
        ach_time = 0;
...
void DrawMessage (void)
{
    ...
    fontnumber = 0;
    SETFONTCOLOR(0xf5,0x0); // set the color
    PrintX=50; PrintY=166; // position the message
...
void GetHint (int hintcount)
{
    ...
    if (lhintnum[mapon] >= 0 && hintcount < 16)
    {
        GetHintFromMap(hintcount);
    }
}
...
void GetHintFromMap(int hintcount)
{
    switch (mapon)
    {
        case 0:
            ...
            {
                case 0:
                    strcpy(gamestate.comphint, "This floor is a reprogramming\n"
                           "facility.");
                    break;
                case 1:
                    strcpy(gamestate.comphint, "It is currently under\nconstruction.");
                    break;
                case 2:
                    strcpy(gamestate.comphint, "Please use caution.");
                    break;
                case 3:
                    strcpy(gamestate.comphint, "And don't push any\nwalls now.");
                    break;
                case 4:
                    strcpy(gamestate.comphint, "Don't wanna get lost do you?");
                    break;
                case 5:
                    strcpy(gamestate.comphint, "Rumor has it there are\nsecret stashes.");
                    break;
                case 6:
                    strcpy(gamestate.comphint, "Don't fool around\nin them, you hear?");
                    break;
                case 7:
                    strcpy(gamestate.comphint, "Logging off now.");
                default:
                    break;
            }
            break;
        case 1:
            ...
            {
                case 0:
                    strcpy(gamestate.comphint, "Workers should report to\n"
                                                "floor 6 of this tower for\n"
                                                "the party.");
                    break;
                case 1:
                    strcpy(gamestate.comphint, "If you do not know how to\n"
                                                "get to floor 6,\n"
                                                "you are not invited");
                    break;
                case 3:
            ...
            break;
        default:
            strcpy(gamestate.comphint, "This terminal is down\nfor maintenance.");
            break;
    }
}

const char * STR_ACH_UNLOCKED = "Achievement Unlocked!";

void SetAchievement (int anum)
{
    SD_PlaySound(ACHIEVEMENTSND);
    achievements_unlocked[anum] = true;
    latest_ach = anum;
    ach_time = 320;
    

    sprintf(achievement_text, "%s\n%s", STR_ACH_UNLOCKED, achievement_strings[anum]);
...
void DrawHint (void)
{
    if(strcmp(gamestate.comphint, "") == 0)
    {
        hinttime = 0;
        return;
    }
    ...
    int h = 0, w = 0, mw = 0, i, len = (int) strlen(gamestate.comphint);
    fontstruct *font;
    ...
    for (i = 0; i < len; i++)
    {
        if (gamestate.comphint[i] == '\n')
        {
            ...
            w += font->width[gamestate.comphint[i]];
    }
    ...
    SETFONTCOLOR (0xf, 0x0);
    US_Print (gamestate.comphint);
}

void DrawAchievement (int anum)
{
    ach_time -= tics;
    
    if (anum >= 0 && anum <= 8)
    {
        if (achievements_unlocked[anum])
        {
            
            fontnumber = 0;
            SETFONTCOLOR(0xa2,0x0);
            int h = 0, w = 0, mw = 0, i, len = (int) strlen(achievement_text);
            fontstruct *font;
            ...
            for (i = 0; i < len; i++)
            {
                if (achievement_text[i] == '\n')
                {
                    ...
                    w += font->width[achievement_text[i]];
            }
            ...
            PrintY = h;
            PrintX = WindowX = 300 - mw;
            
            DrawWindow (WindowX - 5, PrintY - 5, mw + 10, h + 10, 0x18);
            DrawOutline (WindowX - 5, PrintY - 5, mw + 10, h + 10, 0x1f, 0x1f);
            SETFONTCOLOR (0xf, 0x0);
            US_Print (achievement_text);
        }
    }
}

const char      *vendmachine_stuff=
{
    "1 - Candy Bar\n    1 bit - 5 health\n"
    "2 - Apple\n    5 bits - 10 health\n"
    "3 - Apple Pie\n    10 bits - 20 health\n"
...
void DrawVendingMenu (void)
{
    ...
    for (aaa = 0; aaa < len; aaa++)
    {
        if (vendmachine_stuff[aaa] == '\n')
        {
            ...
            w += font->width[vendmachine_stuff[aaa]];
    }
