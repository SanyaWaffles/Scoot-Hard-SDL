...
void PictureGrabber (void)
{
    ...
    static char fname[] = "RSHOT000.BMP";
    
#ifndef _WIN32
    static char scrndir[] = "/screens/";
#else
    static char scrndir[] = "./screens/";
#endif
    
    int strpos, strlength;
    char *ptr1, *ptr2;
    
    sprintf(screenpath, "%s%s", configdir, scrndir);
    
    for (int i = 0; scrndir[i]; i++)
    {
        ptr1 = &scrndir[i];
        ptr2 = scrndir;
    ...
    strlength = strlen(screenpath);
    
    strpos = strpos + (strlength);
    
#ifndef _WIN32
    mkdir(screenpath, S_IRWXU | S_IRWXG | S_IRWXO);
#else
	_mkdir(screenpath);
#endif

    strcat(screenpath, fname);
    
    for(int i = 0; i < 1000; i++)
    {
        screenpath[strpos+7] = i % 10 + '0';
        screenpath[strpos+6] = (i / 10) % 10 + '0';
        screenpath[strpos+5] = i / 100 + '0';
        int file = open(screenpath, O_RDONLY | O_BINARY);
    ...
    SDL_SaveBMP(curSurface, screenpath);
...
int DebugKeys (void)
{
    ...
    else if (Keyboard[sc_I])        // I = item cheat
    {
        ...
        // GivePoints (100000);
        HealSelf (99);
        if (gamestate.bestweapon<wp_rthrower)
...
again:
    ...
    else if (Keyboard[sc_W])        // W = warp to level
    {
        ...
        PrintY+=6;
#ifndef SPEAR
        US_Print("  Warp to which level(1-6): ");
#else
