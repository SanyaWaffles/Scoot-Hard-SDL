...
extern int ach_time, latest_ach;
...
#define TIMEY   8
    ...
    if (!(gamestate.episode == 2 && gamestate.mapon == 4))
    {
        
    
    Write (18, 2, STR_YOUWIN);

    if (gamestate.episode+1 == episodes_unlocked && gamestate.episode < 2)
    {
        Write(1, 4, "new episode awarded!");
        
        episodes_unlocked = gamestate.episode+2;
...
//
...
#endif
    {
...
#else
        Write (26, 12, parTimes[gamestate.episode * 6 + mapon].timestr);
#endif
        ...
        if (gamestate.TimeCount < parTimes[gamestate.episode * 6 + mapon].time * 4200)
            timeleft = (int32_t) ((parTimes[gamestate.episode * 6 + mapon].time * 4200) / 70 - sec);
...
#define RATIOXX                37
        ...
        if (kr >= 100 && sr >= 100 && tr >= 100)
            if (achievements_unlocked[8] == false)
                SetAchievement(8);
...
DrawAchievements (void)
{
    ...
    SETFONTCOLOR (0x4a, 0x2d);
    PrintX = 96;
    PrintY = 15;

    US_Print ("Achievements");
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        if(achievements_unlocked[i])
        {
            ...
            US_PrintSigned(i + 1);
            US_Print (" ");
            US_Print (achievement_strings[i]);
            US_Print (": ");
            US_Print (achievement_defs[i]);
        }
...
#else
    US_Print ("This game is free to share.\n");
...
BetaVersion (void)
{
    ...
    US_Print ("BETA SOFTWARE");
    
    SETFONTCOLOR (HIGHLIGHT, BKGDCOLOR);
    WindowX = PrintX = 40;
    PrintY = 60;
    US_Print ("This game is beta quality.\n\n");
    US_Print ("While it has been tested\n");
    US_Print ("Personally, I cannot guarantee\n");
    US_Print("That it is bug free.\n\n");
    ...
    WindowX = PrintX = 40;
    PrintY = 60;
    US_Print ("For MacOS users, your configs\n");
    US_Print ("are located in your\n");
    US_Print ("user folder, but hidden in\n");
    US_Print (".shard1. If you cannot\n");
    US_Print ("find it, contact me.\n");
