...
const char* COMPASS_MSG_NORTH       = "North";
const char* COMPASS_MSG_NORTHEAST   = "Northeast";
const char* COMPASS_MSG_EAST        = "East";
const char* COMPASS_MSG_SOUTHEAST   = "Southeast";
const char* COMPASS_MSG_SOUTH       = "South";
const char* COMPASS_MSG_SOUTHWEST   = "Southwest";
const char* COMPASS_MSG_WEST        = "West";
const char* COMPASS_MSG_NORTHWEST   = "Northwest";
...
void CheckKeys (void)
{
    ...
    //
    // SECRET CHEAT CODE: 'MLP'
    //
    if (Keyboard[sc_M] && Keyboard[sc_L] && Keyboard[sc_P])
    {
...
//
    ...
    if (Keyboard[sc_C])
    {
        ...
        if (compass)
        {
            if(player->angle < 110 && player->angle >= 70)
            {
                GetMessage(COMPASS_MSG_NORTH);
            }
            else if (player->angle < 70 && player->angle >= 20)
            {
                GetMessage(COMPASS_MSG_NORTHEAST);
            }
            else if (player->angle < 340 && player->angle >= 290)
            {
                GetMessage(COMPASS_MSG_SOUTHEAST);
            }
            else if (player->angle < 290 && player->angle >= 250)
            {
                GetMessage(COMPASS_MSG_SOUTH);
            }
            else if (player->angle < 250 && player->angle >= 200)
            {
                GetMessage(COMPASS_MSG_SOUTHWEST);
            }
            else if (player->angle < 160 && player->angle >= 110)
            {
                GetMessage(COMPASS_MSG_NORTHWEST);
            }
            else if (player->angle < 200 && player->angle >= 160)
            {
                GetMessage(COMPASS_MSG_WEST);
            }
            else
            {
                GetMessage(COMPASS_MSG_EAST);
            }
        }
        return;
    }
...
{
    SD_MusicOff ();
    lastmusicchunk = (musicnames) songs[gamestate.mapon + gamestate.episode * 6];
